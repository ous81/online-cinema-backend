...
if ! command -v jq >/dev/null 2>&1; then
  echo "jq not found — output will be raw JSON. Install with: sudo apt-get install -y jq"
  have_jq=false
fi
...
echo "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO0lBUsAAAAASUVORK5CYII=" | base64 -d > "$TMP_PNG"
